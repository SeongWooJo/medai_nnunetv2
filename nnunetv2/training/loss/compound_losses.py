...
class DC_and_CE_loss2(nn.Module):
    ...
    def safe_cross_entropy(self, logits, target, eps=1e-8):
        log_probs = F.log_softmax(logits, dim=1)  # ✅ log_softmax() 직접 사용
        log_probs = torch.clamp(log_probs, min=torch.log(torch.tensor(eps, device=log_probs.device)))  # ✅ -inf 방지
        loss = F.nll_loss(log_probs, target)  # ✅ 안정적인 cross entropy 계산
        return loss
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        if target.ndim == net_output.ndim:
            ...
            target = target[:, 0]
        ce_loss = self.safe_cross_entropy(net_output, target.long()) if self.weight_ce != 0 and (self.ignore_label is None or num_fg > 0) else 0
