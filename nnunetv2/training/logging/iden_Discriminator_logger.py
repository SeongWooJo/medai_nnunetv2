...
class iden_DiscriminatorLogger(object):
    """
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'domain_acc':
            new_ema_pseudo_dice = self.my_fantastic_logging['domain_acc'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['domain_acc']) > 0 else value
            self.log('ema_accurancy', new_ema_pseudo_dice, epoch)
    ...
    def plot_progress_png(self, output_folder):
        ...
        # classifier loss 그리기기
        ax = ax_all[1]
        ax2 = ax.twinx()
        x_values = list(range(epoch + 1))
        ax.plot(x_values, self.my_fantastic_logging['class_train_losses'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['class_val_losses'][:epoch + 1], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['domain_acc'][:epoch + 1], color='g', ls='dotted', label="domain_acc",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['ema_accurancy'][:epoch + 1], color='g', ls='-', label="Accurancy (mov. avg.)",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
