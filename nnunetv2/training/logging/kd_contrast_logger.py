...
class KDContrastLogger(object):
    ...
    def plot_progress_png(self, output_folder):
        ...
        ax = ax_all[1]
        ax2 = ax.twinx()
        ax.plot(x_values, self.my_fantastic_logging['train_c_losses'][:epoch + 1], color='r', ls='dotted', label="contrast_train", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_c_losses'][:epoch + 1], color='r', ls='-', label="contrast_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['train_kd_losses'][:epoch + 1], color='b', ls='dotted', label="kd_train",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['val_kd_losses'][:epoch + 1], color='b', ls='-', label="kd_val",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("contrast_loss")
        ax2.set_ylabel("kd_loss")
        ax.legend(loc=(0, 1))
        ax2.legend(loc=(0.2, 1))
...
#     # test variable
#     num_epochs = 20
#     logger.my_fantastic_logging['train_losses'] = np.linspace(1.0, 0.1, num_epochs).tolist()
#     logger.my_fantastic_logging['student_train_losses'] = np.linspace(1.2, 0.1, num_epochs).tolist()
#     logger.my_fantastic_logging['teacher_train_losses'] = np.linspace(1.0, 0.05, num_epochs).tolist()
#     logger.my_fantastic_logging['val_losses'] = np.linspace(1.2, 0.15, num_epochs).tolist()
#     logger.my_fantastic_logging['mean_fg_dice'] = np.linspace(0.5, 0.85, num_epochs).tolist()
#     logger.my_fantastic_logging['ema_fg_dice'] = np.linspace(0.48, 0.83, num_epochs).tolist()
#     logger.my_fantastic_logging['epoch_start_timestamps'] = list(range(0, 200, 10))
#     logger.my_fantastic_logging['epoch_end_timestamps'] = list(range(10, 210, 10))
#     logger.my_fantastic_logging['lrs'] = np.linspace(0.01, 0.0001, num_epochs).tolist()
#     logger.my_fantastic_logging['dice_per_class_or_region'] = np.linspace(0.4, 0.6, num_epochs).tolist()

#     logger.plot_progress_png(output_folder='./')