...
class DiscriminatorLogger(object):
    """
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'accurancy':
            new_ema_pseudo_dice = self.my_fantastic_logging['accurancy'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['accurancy']) > 0 else value
    ...
    def plot_progress_png(self, output_folder):
        ...
        ax2.plot(x_values, self.my_fantastic_logging['accurancy'][:epoch + 1], color='g', ls='dotted', label="accurancy",
                 linewidth=3)
