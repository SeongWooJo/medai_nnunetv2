...
from nnunetv2.training.logging.kd_logger import KD_Logger
...
class KDFeatured_Trainer_cascade(nnUNetTrainer):
    ...
    def _build_t_loss(self):
        ...
        loss = KD_KLDiv_loss(reduction = 'mean', temperature = 5)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(list(self.network.parameters()), self.initial_lr, weight_decay=self.weight_decay, # + list(self.teacher_network.embedding_layer.parameters())
    ...
    def on_train_epoch_start(self):
        ...
        self.tumor_queue = deque(maxlen=self.queue_length)
        self.kidney_queue = deque(maxlen=self.queue_length)
        self.background_queue = deque(maxlen=self.queue_length)


    def train_step(self, batch: dict) -> dict:
        temp_data = batch['data']
        temp_target = batch['target']
        
        target = [target_elem[:, 0:1, :, :, :] for target_elem in temp_target]
        cascade_mask = [target_elem[:, 1:2, :, :, :] for target_elem in temp_target]
        #print(temp_target[0].shape)
        data_0 = torch.cat([temp_data[:, 0:1, :, :, :], cascade_mask[0]], dim=1)   # NCCT ch0 + cascade img
        data_1 = torch.cat([temp_data[:, 1:2, :, :, :], cascade_mask[0]], dim=1)   # CECT ch1 + cascade img
        ...
        if isinstance(target, list):
            ...
            cascade_mask = [i.to(self.device, non_blocking=True) for i in cascade_mask]
        else:
            target = target.to(self.device, non_blocking=True)
            cascade_mask = cascade_mask.to(self.device, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            teacher_output, _, teacher_embeded_feature = self.teacher_network(data_1)           # CECT, return result, features, embedding_features
            
            # Forward pass with the student model
            student_output, _, student_embeded_feature = self.network(data_0)                       # NCCT
            
            foreground_output_s = [torch.where(cascade_mask_elem==1, 
                                               student_output_elem, 
                                               torch.full_like(student_output_elem, -100, dtype=torch.float32)
                                               ) for cascade_mask_elem, student_output_elem in zip(cascade_mask, student_output)]
            
            output_s = [torch.cat([torch.where(cascade_mask_elem==1, 
                                               torch.full_like(cascade_mask_elem, -100, dtype=torch.float32), 
                                               torch.zeros_like(cascade_mask_elem, dtype=torch.float32)
                                               ),
                                    foreground_output_elem], dim=1
                                    ) for cascade_mask_elem, foreground_output_elem in zip(cascade_mask, foreground_output_s)]
            
            student_l = self.loss(output_s, target)               # DC_CE_loss(student network output, gt label)
        '''
            # foreground_embeded_features_t = [torch.where(cascade_mask_elem==1, 
            #                                              embeded_elem,
            #                                              torch.full_like(embeded_elem, 0, dtype=torch.float32)
            #                                             ) for cascade_mask_elem, embeded_elem in zip(cascade_mask, teacher_embeded_feature)]

            # foreground_embeded_features_s = [torch.where(cascade_mask_elem==1, 
            #                                              embeded_elem,
            #                                              torch.full_like(embeded_elem, 0, dtype=torch.float32)
            #                                             ) for cascade_mask_elem, embeded_elem in zip(cascade_mask, student_embeded_feature)]

            # embeded_features_t = [torch.cat([torch.where(
            #                         cascade_mask_elem==1, 
            #                         torch.full_like(cascade_mask_elem, -100, dtype=torch.float32), 
            #                         torch.zeros_like(cascade_mask_elem, dtype=torch.float32),
            #                         ),
            #                         foreground_elem], dim=1) for cascade_mask_elem, foreground_elem in zip(cascade_mask, foreground_embeded_features_t)]

            # embeded_features_s = [torch.cat([torch.where(
            #                         cascade_mask_elem==1, 
            #                         torch.full_like(cascade_mask_elem, -100, dtype=torch.float32), 
            #                         torch.zeros_like(cascade_mask_elem, dtype=torch.float32),
            #                         ),
            #                         foreground_elem], dim=1) for cascade_mask_elem, foreground_elem in zip(cascade_mask, foreground_embeded_features_s)]
        ...
        kd_loss_list = []
        # for weight, embeded_feature_t, embeded_feature_s in zip(weights, foreground_embeded_features_t, foreground_embeded_features_s):
        for weight, embeded_feature_t, embeded_feature_s in zip(weights, teacher_embeded_feature, student_embeded_feature):
            # teacher_norm_feature = torch.nn.functional.normalize(teacher_embeded_features, p=2, dim=1)
            # student_norm_feature = torch.nn.functional.normalize(student_embeded_features, p=2, dim=1)
            # mse_loss = torch.nn.functional.mse_loss(teacher_norm_feature, student_norm_feature)
            mse_loss = torch.nn.functional.mse_loss(embeded_feature_t, embeded_feature_s)
        ...
        student_weight = 0.75
        kd_weight = 0.25
        # l = student_weight * student_l + kd_weight * kd_loss
        kd_loss = 0
        l = student_l
        ...
        return {'compound loss': l.detach().cpu().numpy(),
                'student loss': student_l.detach().cpu().numpy(),
                'teacher loss': kd_loss}     # 'teacher loss': kd_loss.detach().cpu().numpy()}
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            loss_here = np.mean(outputs['compound loss'])
            loss_here_student = np.mean(outputs['student loss'])
            loss_here_teacher = np.mean(outputs['teacher loss'])
        torch.cuda.empty_cache()
        self.logger.log('train_losses', loss_here, self.current_epoch)
        self.logger.log('student_train_losses', loss_here_student, self.current_epoch)
        self.logger.log('teacher_train_losses', loss_here_teacher, self.current_epoch)
        self.tumor_queue.clear()
        self.kidney_queue.clear()
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        data = torch.cat([temp_data[:, 0:1, :, :, :], cascade_mask[0]], dim=1)
        ...
        # So autocast will only be active if we have a cuda device.
        with autocast(self.device.type, enabled=True) if 0 else dummy_context():
            temp_output, _, _  = self.network(data)
            foreground_output = [
                        torch.where(
                            cascade_mask_elem==1, 
                            temp_output_elem, 
                        ...
                        for cascade_mask_elem, temp_output_elem in zip(cascade_mask, temp_output)
                    ]
            output = [
                        torch.cat([
                            torch.where(
                            ...
                            foreground_output_elem], dim=1) 
                            for cascade_mask_elem, foreground_output_elem in zip(cascade_mask,foreground_output)
                    ]
            del data
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('student_train_loss', np.round(self.logger.my_fantastic_logging['student_train_losses'][-1], decimals=4))
        self.print_to_log_file('teacher_train_loss', np.round(self.logger.my_fantastic_logging['teacher_train_losses'][-1], decimals=4))
