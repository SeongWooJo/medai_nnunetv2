...
class featured_embedding_target_cos_similiar_Trainer(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if 0 else dummy_context():
            ...
            teacher_output, teacher_features = self.teacher_network(data_1)           # CECT
            
            # Forward pass with the student model
            student_output, student_features = self.network(data_0)                       # NCCT
        ...
        for output, target_elem in zip(student_output, target):
            ...
            kidney_masks.append(current_kidney_mask)
        ...
        for weight, kidney_mask, tumor_mask, tumor_wrong_pred_mask, teacher_feature, student_feature in zip(weights, kidney_masks, tumor_masks, tumor_wrong_pred_masks, teacher_features, student_features):
            kidney_positions = kidney_mask.squeeze(dim=1).nonzero(as_tuple=False)
            tumor_positions = tumor_wrong_pred_mask.squeeze(dim=1).nonzero(as_tuple=False)
            #print(len(tumor_positions))
            ...
            if contrast_condition:
                ...
                for batch_num in range(0,batch_size):
                    if len(batch_tumor_positions[batch_num]) == 0 or len(batch_kidney_positions[batch_num]) == 0:
                        continue
                    random_sample_nums = int(1e+7) // len(batch_tumor_positions[batch_num])
            
                    sample_tumor_pos = batch_tumor_positions[batch_num]
                    
                    tumor_depth_idx, tumor_height_idx, tumor_width_idx = sample_tumor_pos[:, 1], sample_tumor_pos[:, 2], sample_tumor_pos[:, 3]
                        
                    teacher_tumor_vector = teacher_tumor_feature[batch_num, :, tumor_depth_idx, tumor_height_idx, tumor_width_idx].T
                    student_tumor_vector = student_tumor_feature[batch_num, :, tumor_depth_idx, tumor_height_idx, tumor_width_idx].T
                    batch_teacher_tumor_vectors[batch_num] = teacher_tumor_vector
                    batch_student_tumor_vectors[batch_num] = student_tumor_vector
                    
                    #kidney_idx = batch_kidney_positions[batch_num].repeat(len(tumor_idx), 1)
                    expanded_scale = 0
                    if len(batch_kidney_positions[batch_num]) < random_sample_nums:
                        kidney_idx = torch.arange(len(batch_kidney_positions[batch_num]), device=self.device).repeat(len(sample_tumor_pos),)
                        sample_kidney_pos = batch_kidney_positions[batch_num][kidney_idx]
                        expanded_scale = len(batch_kidney_positions[batch_num])
                    else:    
                        kidney_idx = torch.randint(0, len(batch_kidney_positions[batch_num]),(random_sample_nums,), device=self.device)
                        expanded_kidney_idx = kidney_idx.repeat(len(sample_tumor_pos),)
                        sample_kidney_pos = batch_kidney_positions[batch_num][expanded_kidney_idx]
                        expanded_scale = random_sample_nums
                    expanded_tumor_idx = torch.arange(len(sample_tumor_pos), device=self.device).unsqueeze(1).expand(-1, expanded_scale).reshape(-1)
                    expanded_tumor_pos = sample_tumor_pos[expanded_tumor_idx]
                    ...
                    student_kidney_vector = student_kidney_feature[batch_num, :, kidney_depth_idx, kidney_height_idx, kidney_width_idx].T  # (c,)
                    expanded_student_tumor_vector = student_tumor_feature[batch_num, :, e_tumor_depth_idx, e_tumor_height_idx, e_tumor_width_idx].T
                    teacher_kidney_tensor = student_kidney_vector.view(len(sample_tumor_pos), expanded_scale, -1)
                    expanded_student_tumor_tensor = expanded_student_tumor_vector.view(len(sample_tumor_pos), expanded_scale, -1)
                    
                    kidney_similarity = torch.nn.functional.cosine_similarity(teacher_kidney_tensor, expanded_student_tumor_tensor, dim=-1)
                    if kidney_pairs_nums > kidney_similarity.size(1):
                        temp_top_kidney_similarity, _ = kidney_similarity.topk(kidney_similarity.size(1), dim=1)
                        padding_size = max(0, kidney_pairs_nums - kidney_similarity.size(1))
                        top_kidney_similarity = torch.nn.functional.pad(temp_top_kidney_similarity, (padding_size, 0), "constant", 0)
                    else:
                        top_kidney_similarity, _ = kidney_similarity.topk(kidney_pairs_nums, dim=1)
                    batch_kidney_similarity[batch_num] = top_kidney_similarity
                ...
                sim_loss = -1 /(len(tumor_positions)) * torch.log(exp_t.sum() / exp_k.sum())
            else:
