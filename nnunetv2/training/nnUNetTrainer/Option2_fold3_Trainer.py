...
from nnunetv2.training.loss.contrast_loss import NCCT_ContrastLoss
...
class Option2_fold3_Trainer(nnUNetTrainer):
    ...
    def build_teacher_network_architecture(architecture_class_name: str,
        ...
        pretrain_network = torch.load("/data/seongwoo/nnunetFrame/nnunet_results/Dataset602_Kits19_fg_concat_CECT/IgnoreTrainer__nnUNetPlans__3d_cascade_fullres/fold_3/checkpoint_final.pth", weights_only=False)['network_weights']
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            foreground_output_t = [torch.where(cascade_mask_elem==1, 
                                               teacher_output_elem, 
                                               torch.full_like(teacher_output_elem, -100, dtype=torch.float32)
                                               ) for cascade_mask_elem, teacher_output_elem in zip(cascade_mask, teacher_output)]
            
            output_t = [torch.cat([torch.where(cascade_mask_elem==1, 
                                               torch.full_like(cascade_mask_elem, -100, dtype=torch.float32), 
                                               torch.zeros_like(cascade_mask_elem, dtype=torch.float32)
                                               ),
                                    foreground_output_elem], dim=1
                                    ) for cascade_mask_elem, foreground_output_elem in zip(cascade_mask, foreground_output_t)]
            student_l = self.loss(output_s, target) 
            kd_loss = self.t_loss(output_s, output_t)               # DC_CE_loss(student network output, gt label)
            sim_loss = self.c_loss(student_output, student_embeded_feature, target, self.kidney_queue)
        ...
        l = student_weight * student_l + kd_weight * kd_loss + sim_weight * sim_loss
