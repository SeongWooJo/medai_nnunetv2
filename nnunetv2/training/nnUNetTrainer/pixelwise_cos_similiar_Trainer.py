...
class pixelwise_cos_similiar_Trainer(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if 0 else dummy_context():
            ...
            if contrast_condition:
                ...
                tumor_similarity = torch.nn.functional.cosine_similarity(teacher_tumor_tensor, student_tumor_tensor, dim=-1) * tumor_distance_tensor
                kidney_similarity = torch.nn.functional.cosine_similarity(teacher_kidney_tensor, student_tumor_tensor, dim=-1) * kidney_distance_tensor
