...
class featured_embedding_simple_contrast_Trainer(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        ### Contrast Learning
        for idx, (weight, kidney_mask, tumor_mask, tumor_wrong_pred_mask, student_feature) in enumerate(zip(weights, kidney_masks, tumor_masks, tumor_wrong_pred_masks, student_features)):
            ...
            if not len(tumor_wrong_positions) == 0:
                ...
                for batch_num in range(0,batch_size):
                    ...
                    if not len(batch_tumor_positions[batch_num]) == 0 and not len(self.kidney_queue[idx]) == 0 and not len(batch_target_positions[batch_num]) == 0:
